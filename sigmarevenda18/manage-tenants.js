...
async function showStats(db) {
    ...
    const stats = await get(db, `
        SELECT 
            ...
            (SELECT COUNT(*) FROM p2bras_panels WHERE tenant_id = ?) as p2bras_panels,
            (SELECT COUNT(*) FROM rushplay_panels WHERE tenant_id = ?) as rushplay_panels,
            (SELECT COUNT(*) FROM painelfoda_panels WHERE tenant_id = ?) as painelfoda_panels,
            (SELECT COUNT(*) FROM dashboardbz_panels WHERE tenant_id = ?) as dashboardbz_panels,
    ...
    const resellers = await get(db, `
        SELECT 
            ...
            (SELECT COUNT(*) FROM gesoffice_resellers gr JOIN gesoffice_panels gp ON gr.panel_id = gp.id WHERE gp.tenant_id = ?) +
            (SELECT COUNT(*) FROM p2bras_resellers pr JOIN p2bras_panels pp ON pr.panel_id = pp.id WHERE pp.tenant_id = ?) +
            (SELECT COUNT(*) FROM rushplay_resellers rpr JOIN rushplay_panels rpp ON rpr.panel_id = rpp.id WHERE rpp.tenant_id = ?) +
            (SELECT COUNT(*) FROM painelfoda_resellers pfr JOIN painelfoda_panels pfp ON pfr.panel_id = pfp.id WHERE pfp.tenant_id = ?) +
            (SELECT COUNT(*) FROM dashboardbz_resellers dbr JOIN dashboardbz_panels dbp ON dbr.panel_id = dbp.id WHERE dbp.tenant_id = ?)
            as total
    ...
    console.log(`  Painéis P2BRAS: ${stats.p2bras_panels}`);
    console.log(`  Painéis RushPlay: ${stats.rushplay_panels}`);
    console.log(`  Painéis PainelFoda: ${stats.painelfoda_panels}`);
    console.log(`  Painéis DashboardBz: ${stats.dashboardbz_panels}`);
    const totalPanels = stats.sigma_panels + stats.koffice_panels + stats.gesoffice_panels + 
                        stats.p2bras_panels + stats.rushplay_panels + stats.painelfoda_panels + 
                        stats.dashboardbz_panels;
    console.log(`  Total de Painéis: ${totalPanels}`);
