// js/client/packages.js - VERSÃO CORRIGIDA COMPLETA
const ClientPackages = {
    selectedPackage: null,

    async loadPackages() {
        try {
            // CORREÇÃO: Buscar dados de ClientAuth.currentReseller
            if (!ClientAuth.currentReseller) {
                console.error('Nenhum revendedor logado');
                return;
            }

            const resellerId = ClientAuth.currentReseller.id;
            const type = ClientAuth.currentReseller.type;

            console.log(`📦 Carregando pacotes - Revendedor ID: ${resellerId}, Tipo: ${type}`);
            ...
            console.log(`✅ ${packages.length} pacotes carregados`);
        } catch (error) {
