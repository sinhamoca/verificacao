...
// Gerenciamento de revendedores GesOffice (UNIPLAY)
...
const AdminGesOfficeResellers = {
    ...
    async render(resellers) {
        ...
        if (resellers.length === 0) {
            tbody.innerHTML = '<tr><td colspan="6" style="text-align: center; color: #888;">Nenhum revendedor UNIPLAY cadastrado</td></tr>';
            return;
        }

        for (const reseller of resellers) {
            const packages = await API.admin.getPackages(reseller.id, 'gesoffice');
            const statusBadge = Components.createStatusBadge(reseller.status);
            
            const row = document.createElement('tr');
            row.innerHTML = `
                <td>${reseller.username}</td>
                <td><code>${reseller.gesoffice_id}</code></td>
