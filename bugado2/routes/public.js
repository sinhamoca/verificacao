// routes/public.js - COM SUPORTE MULTI-TENANT + EXPIRAÇÃO + SIGMA + KOFFICE + GESOFFICE + P2BRAS
...
module.exports = (db) => {
    ...
    router.post('/verify-access', async (req, res) => {
        try {
            ...
            if (!config) {
                return res.status(403).json({ error: 'Senha não configurada' });
            }
            
            const hashedAnswer = crypto.createHash('sha256').update(answer).digest('hex');
            
            if (hashedAnswer !== config.value) {
    ...
    // ========================================
    // VERIFICAR REVENDEDOR (COM VALIDAÇÃO DE EXPIRAÇÃO)
    // ========================================
    router.post('/verify-reseller', async (req, res) => {
        try {
            ...
            if (tenantSlug) {
                ...
                if (!tenant) {
                    console.log(`[Login] ⚠️ Tenant inválido/expirado: ${tenantSlug}`);
                    return res.status(403).json({ 
                        error: 'Sistema não disponível',
                        code: 'TENANT_UNAVAILABLE'
                    });
            ...
            // ⭐ 4. Se não encontrou, tentar em P2BRAS
            if (!reseller) {
                reseller = await db.get(`
                    SELECT pr.*, pp.name as panel_name, pp.url as panel_url, pp.tenant_id, 'p2bras' as type,
                           t.expires_at, t.status as tenant_status
    ...
    router.get('/packages/:resellerId', async (req, res) => {
        try {
            const { type } = req.query; // sigma, koffice, gesoffice ou p2bras
    ...
    router.post('/create-payment', async (req, res) => {
        try {
            ...
            // Buscar informações do revendedor baseado no tipo
    ...
    router.post('/check-payment/:paymentId', async (req, res) => {
        try {
            ...
            if (mpPayment.status === 'approved') {
                // Importar MonitorService para processar
