...
class Database {
    ...
    initTables() {
        this.db.serialize(() => {
            ...
            // Painéis RushPlay
            this.db.run(`CREATE TABLE IF NOT EXISTS rushplay_panels (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                tenant_id INTEGER DEFAULT 1,
                name TEXT NOT NULL,
                url TEXT NOT NULL,
            ...
            this.db.run(`CREATE INDEX IF NOT EXISTS idx_rushplay_panels_tenant 
                ON rushplay_panels(tenant_id)`);

            // Revendedores RushPlay
            this.db.run(`CREATE TABLE IF NOT EXISTS rushplay_resellers (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                username TEXT NOT NULL,
                rushplay_id TEXT NOT NULL,
                panel_id INTEGER NOT NULL,
                status TEXT DEFAULT 'active',
                created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
                FOREIGN KEY (panel_id) REFERENCES rushplay_panels (id) ON DELETE CASCADE
            )`);

            this.db.run(`CREATE INDEX IF NOT EXISTS idx_rushplay_resellers_username 
                ON rushplay_resellers(username)`);
            ...
            // Painéis PainelFoda
            this.db.run(`CREATE TABLE IF NOT EXISTS painelfoda_panels (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                tenant_id INTEGER DEFAULT 1,
                name TEXT NOT NULL,
                url TEXT NOT NULL,
            ...
            this.db.run(`CREATE INDEX IF NOT EXISTS idx_painelfoda_panels_tenant 
                ON painelfoda_panels(tenant_id)`);

            // Revendedores PainelFoda
            this.db.run(`CREATE TABLE IF NOT EXISTS painelfoda_resellers (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                username TEXT NOT NULL,
                painelfoda_user_id TEXT NOT NULL,
                panel_id INTEGER NOT NULL,
                status TEXT DEFAULT 'active',
                created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
                FOREIGN KEY (panel_id) REFERENCES painelfoda_panels (id) ON DELETE CASCADE
            )`);

            this.db.run(`CREATE INDEX IF NOT EXISTS idx_painelfoda_resellers_username 
                ON painelfoda_resellers(username)`);
            this.db.run(`CREATE INDEX IF NOT EXISTS idx_painelfoda_resellers_panel 
                ON painelfoda_resellers(panel_id)`);

            // PAINÉIS DASHBOARDBZ
            this.db.run(`CREATE TABLE IF NOT EXISTS dashboardbz_panels (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                tenant_id INTEGER DEFAULT 1,
                name TEXT NOT NULL,
                url TEXT NOT NULL,
            ...
            this.db.run(`CREATE INDEX IF NOT EXISTS idx_dashboardbz_panels_tenant 
                ON dashboardbz_panels(tenant_id)`);

            // REVENDEDORES DASHBOARDBZ
            this.db.run(`CREATE TABLE IF NOT EXISTS dashboardbz_resellers (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                username TEXT NOT NULL,
                dashboardbz_search_term TEXT NOT NULL,
                panel_id INTEGER NOT NULL,
                status TEXT DEFAULT 'active',
                created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
                FOREIGN KEY (panel_id) REFERENCES dashboardbz_panels (id) ON DELETE CASCADE
            )`);

            this.db.run(`CREATE INDEX IF NOT EXISTS idx_dashboardbz_resellers_username 
                ON dashboardbz_resellers(username)`);
            this.db.run(`CREATE INDEX IF NOT EXISTS idx_dashboardbz_resellers_panel 
                ON dashboardbz_resellers(panel_id)`);
    ...
    async getValidTenant(slug) {
        console.log(`[getValidTenant] 🔍 Buscando: ${slug}`);
        
        // Primeiro buscar o tenant sem filtros
        const tenant = await this.get(`
            SELECT * FROM tenants WHERE slug = ?
        `, [slug]);
        
        if (!tenant) {
            console.log(`[getValidTenant] ❌ Tenant não existe: ${slug}`);
            return null;
        }
        
        console.log(`[getValidTenant] 📋 Tenant encontrado:`, {
            id: tenant.id,
            name: tenant.name,
            slug: tenant.slug,
        ...
        if (tenant.status !== 'active') {
            console.log(`[getValidTenant] ❌ Status inativo: ${tenant.status}`);
            return null;
        }
        ...
        if (tenant.expires_at) {
            ...
            console.log(`[getValidTenant] 📅 Hoje: ${now} | Expira: ${tenant.expires_at}`);
            
            if (tenant.expires_at < now) {
                console.log(`[getValidTenant] ❌ Expirado`);
                return null;
            }
        }
        
        console.log(`[getValidTenant] ✅ Tenant válido!`);
        return tenant;
    }
