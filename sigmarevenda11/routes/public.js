// routes/public.js - COM SUPORTE MULTI-TENANT + EXPIRAÇÃO + SIGMA + KOFFICE + GESOFFICE
...
module.exports = (db) => {
    ...
    // ========================================
    // PERGUNTA DE ACESSO (COM VALIDAÇÃO DE EXPIRAÇÃO)
    // ========================================
    router.get('/access-question', async (req, res) => {
        try {
            ...
            if (tenantSlug) {
                // ⭐ USAR getValidTenant QUE JÁ VALIDA EXPIRAÇÃO
                const tenant = await db.getValidTenant(tenantSlug);
                
                if (!tenant) {
                    console.log(`[Access Question] ⚠️ Tenant inválido/expirado: ${tenantSlug}`);
                    return res.status(403).json({ 
                        error: 'Sistema não disponível. Entre em contato com o suporte.',
            ...
            console.error('[Access Question] Erro:', error);
            res.status(500).json({ error: error.message });
        }
    });

    // ========================================
    // VERIFICAR SENHA DE ACESSO (COM VALIDAÇÃO DE EXPIRAÇÃO)
    // ========================================
    router.post('/verify-access', async (req, res) => {
        try {
            ...
            if (tenantSlug) {
                ...
                if (!tenant) {
                    console.log(`[Verify Access] ⚠️ Tenant inválido/expirado: ${tenantSlug}`);
                    return res.status(403).json({ 
    ...
    // ========================================
    // LOGIN UNIFICADO (COM VALIDAÇÃO DE EXPIRAÇÃO)
    // ========================================
    router.post('/login', async (req, res) => {
        try {
            ...
            if (tenantSlug) {
                // ⭐ VALIDAR TENANT ANTES DE BUSCAR REVENDEDOR
                const tenant = await db.getValidTenant(tenantSlug);
                
                if (!tenant) {
                    const tenantInfo = await db.get(
                        'SELECT status, expires_at FROM tenants WHERE slug = ?',
                        [tenantSlug]
                    );
                    
                    const now = new Date().toISOString().split('T')[0];
                    const isExpired = tenantInfo?.expires_at && tenantInfo.expires_at < now;
                    
                    console.log(`[Login] ⚠️ Tentativa de login em tenant ${isExpired ? 'expirado' : 'inativo'}: ${tenantSlug}`);
                    
                    return res.status(403).json({ 
                        error: isExpired ? 'Acesso expirado. Entre em contato com o suporte.' : 'Sistema não disponível',
                        code: 'TENANT_UNAVAILABLE'
                    });
                }
    ...
    // ========================================
    // CRIAR PAGAMENTO (COM VALIDAÇÃO DE EXPIRAÇÃO)
    // ========================================
    router.post('/create-payment', async (req, res) => {
        try {
            ...
            // ⭐ BUSCAR REVENDEDOR E VALIDAR TENANT
            let resellerInfo;
            let panelTenantId;
            
            if (resellerType === 'sigma') {
                resellerInfo = await db.get(`
                    SELECT r.username, sp.tenant_id, t.status as tenant_status, t.expires_at
                    FROM resellers r
                    JOIN sigma_panels sp ON r.panel_id = sp.id
                    JOIN tenants t ON sp.tenant_id = t.id
                    WHERE r.id = ?
                ...
                resellerInfo = await db.get(`
                    SELECT kr.username, kp.tenant_id, t.status as tenant_status, t.expires_at
                    FROM koffice_resellers kr
                    JOIN koffice_panels kp ON kr.panel_id = kp.id
                    JOIN tenants t ON kp.tenant_id = t.id
                    WHERE kr.id = ?
                ...
                resellerInfo = await db.get(`
                    SELECT gr.username, gp.tenant_id, t.status as tenant_status, t.expires_at
                    FROM gesoffice_resellers gr
                    JOIN gesoffice_panels gp ON gr.panel_id = gp.id
                    JOIN tenants t ON gp.tenant_id = t.id
                    WHERE gr.id = ?
            ...
            if (!resellerInfo || !resellerInfo.username) {
                console.error(`[Payment] ⚠️ Revendedor não encontrado ou tenant expirado: ID ${resellerId}, Tipo ${resellerType}`);
                return res.status(403).json({ 
                    error: 'Não é possível criar pagamento. Sistema expirado ou indisponível.' 
                });
            }
