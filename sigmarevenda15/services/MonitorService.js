// services/MonitorService.js - COMPLETO COM PAINELFODA + TODOS OS PAINÉIS
...
class MonitorService {
    ...
    async checkPendingPayments() {
        try {
            // ⭐ MULTI-TENANT: Query com TODOS OS PAINÉIS incluindo PainelFoda
            const payments = await this.db.query(`
                SELECT 
                    ...
                    CASE 
                        ...
                        WHEN p.reseller_type = 'painelfoda' THEN pfr.painelfoda_user_id
        ...
        try {
            ...
            // ===== P2BRAS =====
                ...
                const captcha2Config = await this.db.get(
                    'SELECT value FROM tenant_config WHERE tenant_id = ? AND key = ?',
                    [payment.tenant_id, 'captcha2_api_key']
                );
            ...
            // ===== RUSHPLAY =====
                ...
                result = await rushplayService.addCreditsWithRetry(
                    payment.username,
                    payment.user_identifier, // rushplay_id
                    payment.credits
                );
            }
            // ⭐ NOVO: PAINELFODA =====
                ...
                result = await painelfodaService.addCreditsWithRetry(
                    payment.username,
                    payment.user_identifier, // painelfoda_user_id
                    payment.credits
                );
            }
            else {
    ...
    async expirePayment(paymentId) {
        try {
            ...
            console.log(`[Monitor] Pagamento ${paymentId} expirado`);
        } catch (error) {
    ...
    async retryPayment(paymentId) {
        try {
            // ⭐ MULTI-TENANT: Buscar pagamento com informações do painel (incluindo PainelFoda)
