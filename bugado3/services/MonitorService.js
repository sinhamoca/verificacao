// services/MonitorService.js - VERSÃO COMPLETA COM PAINELFODA
...
const PainelFodaService = require('./PainelFodaService'); // ⭐ NOVO
...
class MonitorService {
            ...
            const payments = await this.db.query(`
                SELECT 
                    ...
                    CASE 
                        ...
                        WHEN p.reseller_type = 'p2bras' THEN pp.domain
                        WHEN p.reseller_type = 'rushplay' THEN rpp.domain
                        WHEN p.reseller_type = 'painelfoda' THEN pfp.domain
                    END as url,
                    CASE 
    ...
    async processRecharge(payment, isRetry = false) {
        try {
            ...
            await this.db.run(`
                INSERT INTO transactions (payment_id, reseller_id, reseller_type, credits, amount, sigma_response, success)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            `, [
                payment.id,
                payment.reseller_id,
            ...
            console.log(`[Monitor] ✅ Recarga processada: ${payment.credits} créditos para ${payment.username} (${payment.reseller_type})`);

        } catch (error) {
            console.error(`[Monitor] ❌ Erro ao processar recarga ${payment.id}:`, error.message);

            if (!isRetry) {
                await this.db.run(
                    'UPDATE payments SET status = ?, error_message = ? WHERE id = ?',
                    ['error', error.message, payment.id]
                );
    ...
    async retryPayment(paymentId) {
        try {
            const payment = await this.db.get(`
                SELECT 
                    ...
                    CASE 
                        ...
                        WHEN p.reseller_type = 'painelfoda' THEN pfr.painelfoda_id
            ...
            await this.processRecharge(payment, true);
            
            return {
                success: true,
                message: 'Recarga reprocessada com sucesso!'
            };
