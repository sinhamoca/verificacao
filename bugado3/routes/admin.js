...
module.exports = (db) => {
    ...
    router.post('/painelfoda-panels', requireAuth, async (req, res) => {
        try {
            const { name, domain, admin_username, admin_password } = req.body;
            ...
            const result = await db.run(
                `INSERT INTO painelfoda_panels (tenant_id, name, domain, admin_username, admin_password)
                VALUES (?, ?, ?, ?, ?)`,
                [req.tenantId, name, domain, admin_username, admin_password]
            );

            res.json({
                message: 'Painel PainelFoda cadastrado com sucesso',
                id: result.lastID
            });
        } catch (error) {
            res.status(500).json({ error: error.message });
    ...
    router.put('/painelfoda-panels/:id', requireAuth, async (req, res) => {
        try {
            const { name, domain, admin_username, admin_password, status } = req.body;

            await db.run(
                `UPDATE painelfoda_panels 
                SET name = ?, domain = ?, admin_username = ?, admin_password = ?, status = ?
                WHERE id = ? AND tenant_id = ?`,
                [name, domain, admin_username, admin_password, status, req.params.id, req.tenantId]
            );
    ...
    router.post('/painelfoda-resellers', requireAuth, async (req, res) => {
        try {
            const { username, painelfoda_id, panel_id } = req.body;

            if (!username || !painelfoda_id || !panel_id) {
                return res.status(400).json({ error: 'Todos os campos são obrigatórios' });
            }

            // Verificar se o painel pertence ao tenant
            const panel = await db.get(
            ...
            const existing = await db.get(
                `SELECT pfr.id FROM painelfoda_resellers pfr
                JOIN painelfoda_panels pfp ON pfr.panel_id = pfp.id
                WHERE pfr.username = ? AND pfp.tenant_id = ?`,
            ...
            const result = await db.run(
                `INSERT INTO painelfoda_resellers (username, painelfoda_id, panel_id)
                VALUES (?, ?, ?)`,
                [username, painelfoda_id, panel_id]
            );

            res.json({
                message: 'Revendedor PainelFoda cadastrado com sucesso',
                id: result.lastID
            });
        } catch (error) {
            res.status(500).json({ error: error.message });
    ...
    router.put('/painelfoda-resellers/:id', requireAuth, async (req, res) => {
        try {
            const { username, painelfoda_id, panel_id, status } = req.body;
            ...
            await db.run(
                ...
                SET username = ?, painelfoda_id = ?, panel_id = ?, status = ?
                WHERE id = ? AND panel_id IN (SELECT id FROM painelfoda_panels WHERE tenant_id = ?)`,
                [username, painelfoda_id, panel_id, status, req.params.id, req.tenantId]
            );
    ...
    router.get('/stats', requireAuth, async (req, res) => {
        try {
            // ⭐ MULTI-TENANT: Estatísticas apenas do tenant (COM PAINELFODA)
            ...
            `, [
                req.tenantId, req.tenantId, req.tenantId, req.tenantId, req.tenantId, req.tenantId, // painéis
                req.tenantId, req.tenantId, req.tenantId, req.tenantId, req.tenantId, req.tenantId, // revendedores
                req.tenantId, req.tenantId, req.tenantId // payments
            ]);
            ...
            stats.active_panels = (stats.active_sigma_panels || 0) + 
                                ...
                                (stats.active_painelfoda_panels || 0); // ⭐ NOVO
            ...
            stats.active_resellers = (stats.active_sigma_resellers || 0) + 
                                    ...
                                    (stats.active_painelfoda_resellers || 0); // ⭐ NOVO
    ...
    router.get('/transactions', requireAuth, async (req, res) => {
        try {
            // ⭐ MULTI-TENANT: Filtrar transações por tenant (COM PAINELFODA)
            const transactions = await db.query(`
                ...
                WHERE (sp.tenant_id = ? OR kp.tenant_id = ? OR gp.tenant_id = ? OR pp.tenant_id = ? OR rpp.tenant_id = ? OR pfp.tenant_id = ?)
                ORDER BY t.created_at DESC
                LIMIT 100
            `, [req.tenantId, req.tenantId, req.tenantId, req.tenantId, req.tenantId, req.tenantId]);
