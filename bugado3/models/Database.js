...
class Database {
    ...
    initTables() {
        this.db.serialize(() => {
            ...
            this.db.run(`CREATE INDEX IF NOT EXISTS idx_painelfoda_panels_status 
                ON painelfoda_panels(status)`);

            // ========================================
            // PAINELFODA - REVENDEDORES
            // ========================================
            this.db.run(`CREATE TABLE IF NOT EXISTS painelfoda_resellers (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                username TEXT NOT NULL,
                painelfoda_id INTEGER NOT NULL,
                panel_id INTEGER NOT NULL,
                status TEXT DEFAULT 'active',
                created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
            ...
            this.db.run(`CREATE INDEX IF NOT EXISTS idx_painelfoda_resellers_status 
                ON painelfoda_resellers(status)`);
